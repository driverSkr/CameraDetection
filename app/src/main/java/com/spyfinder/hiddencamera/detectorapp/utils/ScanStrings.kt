...
object ScanStrings {
    ...
    private val exact = mapOf(
        ...
        "Likely storage device" to R.string.identity_storage,
        "Device name suggests type; verify manually" to R.string.identity_name_basis,
        "Conflicting identity clues" to R.string.identity_conflict,
        "Reported identity not matched" to R.string.identity_unmatched,
        "Network services only; no device identity" to R.string.identity_services_only,
        "Online response only; no device identity" to R.string.identity_online_only,
        "mDNS service type" to R.string.identity_mdns_basis,
        "Device type unconfirmed" to R.string.identity_unconfirmed,
        "Media playback device" to R.string.identity_renderer,
        "Media server" to R.string.identity_server,
        "Printer" to R.string.identity_printer,
        ...
        "Authentication required" to R.string.identity_auth,
        "Device description unavailable" to R.string.identity_unavailable,
        ...
        "Background scan protection is unavailable. Keep the app open until the scan finishes." to R.string.scan_unprotected_keep_open,
        "Scan stopped because background protection is unavailable. Results are incomplete. Keep the app open and retry." to R.string.scan_unprotected_background,
        "Scan cancelled to start magnetic detection." to R.string.scan_cancelled_magnetic,
        "Scan cancelled to start the camera." to R.string.scan_cancelled_camera,
    ...
    internal fun format(value: String, lookup: (Int, List<String>) -> String): String {
        ...
        if (value.startsWith("Scan completed. ") || value.startsWith("Partially completed. ")) {
            val prefix = if (value.startsWith("Scan completed. ")) "Scan completed. " else "Partially completed. "
            var result = " " + value.removePrefix(prefix)
            templates.forEach { (pattern, id) ->
                result = pattern.replace(result) { lookup(id, it.groupValues.drop(1)) }
            }
            listOf(" Coverage or evidence storage was limited; review the recorded scope.",
            ...
            return lookup(exact.getValue(prefix), emptyList()) + result.trimStart()
        }
